      ...
      <div className="flex h-screen overflow-hidden">
        ...
        >
          <div className="shrink-0 border-b border-white/10 px-3 py-4">
            <div
              className={`flex items-center gap-3 ${
                isSidebarCollapsed ? "justify-center" : "justify-center md:justify-between"
              }`}
            >
              ...
              <Button
                ...
                className={`h-10 w-10 shrink-0 text-slate-300 hover:bg-white/10 hover:text-white ${
                  isSidebarCollapsed ? "" : "max-md:hidden"
                }`}
                onClick={() => {
          ...
          <nav className="flex-1 space-y-6 overflow-y-auto px-3 py-5">
            {navSections.map((section) => {
              ...
              return (
                <div key={section.id}>
                  {!isSidebarCollapsed ? (
                    <p className="mb-2 hidden px-3 text-[11px] font-semibold uppercase tracking-wide text-slate-500 md:block">
                      {section.label}
                    </p>
                  ) : null}
                  <div className="space-y-2">
                    {sectionItems.map((item) => {
                      const navClassName = `h-12 w-full rounded-lg text-base font-medium ${
                        isSidebarCollapsed
                          ? "justify-center px-0"
          ...
          <div className="shrink-0 border-t border-white/10 p-3">
            <Button
              variant="ghost"
              className={`h-12 w-full text-base font-medium text-slate-300 hover:bg-white/10 hover:text-white ${
                isSidebarCollapsed ? "justify-center px-0" : "justify-center px-0 md:justify-start md:gap-3"
              }`}
              onClick={() => {
                if (window.innerWidth < 768) {
            ...
            >
              ...
              {!isSidebarCollapsed ? <span className="hidden md:inline">Collapse</span> : null}
            </Button>
          </div>
        </aside>
        ...
        <section className="flex h-screen min-w-0 flex-1 flex-col overflow-hidden">
          <header className="flex min-h-16 shrink-0 items-center justify-between gap-3 border-b bg-white px-4 py-3 sm:px-6">
            <div className="flex min-w-0 flex-wrap items-center gap-2 text-base font-medium text-slate-600 sm:gap-3">
              <Button
                size="icon"
                variant="outline"
...
function AurynLogo({ collapsed }: { collapsed: boolean }) {
  return (
    <div className={`flex min-w-0 items-center gap-3 ${collapsed ? "justify-center" : ""}`}>
      <div className={`relative shrink-0 ${collapsed ? "h-7 w-7" : "h-7 w-7 md:h-10 md:w-10"}`}>
        <span className={`absolute rounded-full bg-violet-400 shadow-[0_0_18px_rgba(139,92,246,0.45)] ${collapsed ? "left-3 top-0 h-3.5 w-2" : "left-3 top-0 h-3.5 w-2 md:left-4 md:h-5 md:w-2.5"}`} />
        <span className={`absolute rounded-full bg-cyan-400 shadow-[0_0_18px_rgba(34,211,238,0.35)] ${collapsed ? "bottom-0 left-3 h-3.5 w-2" : "bottom-0 left-3 h-3.5 w-2 md:left-4 md:h-5 md:w-2.5"}`} />
        <span className={`absolute rounded-full bg-blue-500 shadow-[0_0_18px_rgba(59,130,246,0.35)] ${collapsed ? "left-0 top-3 h-2 w-3.5" : "left-0 top-3 h-2 w-3.5 md:top-4 md:h-2.5 md:w-5"}`} />
        <span className={`absolute rounded-full bg-emerald-400 shadow-[0_0_18px_rgba(52,211,153,0.35)] ${collapsed ? "right-0 top-3 h-2 w-3.5" : "right-0 top-3 h-2 w-3.5 md:top-4 md:h-2.5 md:w-5"}`} />
        <span className={`absolute rounded-full bg-white/90 ${collapsed ? "left-[10px] top-[10px] h-2 w-2" : "left-[10px] top-[10px] h-2 w-2 md:left-[15px] md:top-[15px] md:h-2.5 md:w-2.5"}`} />
      </div>
      {!collapsed ? (
